---
title: "Loading data via REST-API"
...
We will load DRP data into a Postgres database via a standardized REST interface. For thsis purpose start a local REST-API server in the background (the server can be shut down later with `app$kill()`)
...
> *Note*: The REST-API is interactively documented via a Swagger interface available at `http://127.0.0.1:3840/__docs__/`

## Loading data into the database

### Clinical data

Instantiate a new test patient, disease stage, sample and assay by making a HTTP `POST` request to the database
...
drpr::plot_plate(
  ...
  palette = "harmony1",
  save = TRUE,
  filename = here::here("man/figures/plots/plate_layout.png")
)
```

![](../man/figures/plots/plate_layout.png)
...
drpr::plot_plate(
  ...
  palette = "sunset",
  save = TRUE,
  filename = here::here("man/figures/plots/plate_measurement.png")
)
```

![](../man/figures/plots/plate_measurement.png)
...
➡️ Next, continue with [**Part II: Curve fitting and drug scoring**](analysis.html)